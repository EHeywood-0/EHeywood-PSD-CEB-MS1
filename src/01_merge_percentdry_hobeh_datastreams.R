# combine initial 2 haul out data streams: percent dry and haulout behavior, merge overlapping events
# EIH
...
all = rbind(dat1, dat2) %>% arrange(ptt, s, e)
...
res2L = lapply(X = lagsL, function(x){
  ...
  repeat{
    ...
    if (length(negidx) == 0) {
      ...
      overlaps = overlaps %>%
        group_by(oid) %>%
        mutate(s = min(s), e = max(e), type = 'behavior + percentdry') %>%
        ungroup() %>%
...
write_csv(x = finalHO, file = "./data/L1/haulout/Hg_2019-2023_PercentDry+Behavior_HauloutMethods_Merged.csv")
